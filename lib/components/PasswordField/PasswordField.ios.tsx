  ...
  const enhancedDescription = (
    <>
      {showStrengthIndicator && passwordStr.length > 0 && !error && (
        <View className="gap-1 mt-0.5 ml-1">
          <View className="flex-row justify-between items-center">
            <Text className="text-[13px] text-muted-foreground">Strength:</Text>
            <Text className={`text-[13px] ${strength.textClass}`}>{strength.label}</Text>
          </View>
      ...
      {showMatchIndicator && passwordsMatch && !error && (
        <View className="flex-row items-center gap-1 mt-0.5 ml-1">
          <DynamicIcon
            icon={{ sfSymbol: 'checkmark.circle', materialIcon: 'check-circle' }}
            size={12}
            className="text-success"
          />
          <Text className="text-[13px] text-success">Passwords match</Text>
        </View>
      )}
      {description}
    </>
